...
# Suppress PyJWT HMAC key length warning (key length is determined by Duo)
warnings.filterwarnings("ignore", message=".*HMAC key.*below the minimum recommended length.*")
...
# IIS handles TLS termination and forwards requests to Flask over HTTP on loopback.
# ProxyFix ensures Flask sees the original client IP, protocol, and host.
...
# Authenticate user via direct LDAP simple bind over TLS
def authenticate_user(username, password):
    user_dn = f'{username}@{KERBEROS_REALM}'
    ...
    try:
        ...
        logger.error(f"LDAP authentication failed for '{username}': {type(e).__name__}: {e}")
        return False
...
# Main - Flask runs on loopback; IIS handles TLS termination via HttpPlatformHandler
